...
from check_broken import *



class MaincheckbrokenWindow(QMainWindow):
    def __init__(self):
        super(MaincheckbrokenWindow, self).__init__()

        self.ui = Ui_check_broken()
    ...
    def connectdb(self):
        ...
        while True:
            ...
            for item, label in zip(list,labels):
                if item == 7:
                    label.setStyleSheet("background-color: #FF0000;border-radius: 10px;color: #fff;")
                elif item == 6:
                    label.setStyleSheet("background-color: #F8DE22;border-radius: 10px;color: #000000;")
                elif item == 1:
